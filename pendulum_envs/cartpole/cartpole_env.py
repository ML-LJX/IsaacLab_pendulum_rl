...
class CartpoleEnvCfg(DirectRLEnvCfg):
    ...
    initial_pole_angle_range = [1 - 0.08, 1 + 0.08]  # the range in which the pole angle is sampled from on reset [rad]
...
def compute_rewards(
    ...
    # penalty_position = -5.0 * (torch.abs(cart_pos) > 1.0).float()
    # total_reward += penalty_position 
